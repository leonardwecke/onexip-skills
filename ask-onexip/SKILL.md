---
name: ask-onexip
description: Ask which onexip skill or flow fits your situation. A router over the installed onexip skills.
...
1. **`/grill-with-docs`** sharpens the idea by interview. Start here whenever you are **working in a working directory**: it's stateful, retaining what it learns in `CONTEXT.md` and ADRs. (No working directory? Use `/grilling` directly, covered under Standalone. `grill-with-docs` runs the same interview and leaves a paper trail, which makes it the better of the two whenever a repo is there to leave it in.)
...
**`/setup-onexip-skills`**: run before your first engineering flow to configure the issue tracker, triage labels, and doc layout the other skills assume. Custom issue trackers also work.
